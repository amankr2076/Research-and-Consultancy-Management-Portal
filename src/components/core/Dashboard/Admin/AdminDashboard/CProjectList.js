...
const CProjectList = () => {
  ...
  const [projectData,setProjectData]=useState([]);
    ...
    useEffect(()=>{
        const getProjectDetails = async () => {
            ...
            try {
              const res = await getAllConsultancyProjectsDeatils(token);
  ...
  return (
    <div className='w-[1200px] ml-[-100px] rounded-lg shadow-lg py-8 px-8'>
        <h2 className='font-bold mb-4'>All Consultancy Projects</h2>
      ...
      {projectData.map((item,index) => (
        <div key={index}>
          ...
          {item?.c_project_id && <div className="grid grid-cols-11 gap-4 py-2 border-b hover: hover:bg-gray-100 transition duration-200 p-2 rounded">
            <div className="col-span-1">{item?.c_project_id}</div>
            <Link className="col-span-3 cursor-pointer ml-[-40px]" to={`/dashboard/admin/main-page/cprojects/${item?.c_project_id}/${item?.u_id}`}>
                    <div >{item?.c_project_title?.length > 40 ? `${item.c_project_title.slice(0, 40)}...` : item.c_project_title}</div>
            </Link>
