...
const SAConsultancy = () => {
  ...
  return (
    <div className='w-[1200px] ml-[-100px] rounded-lg shadow-lg py-8 px-8'>
        <h2 className='font-bold mb-4'>All Consultancy Agencies</h2>
      ...
      {agencyData.map((item,index) => (
        <div key={index}>
          ...
          {item?.c_agency_id && <div className="grid grid-cols-5 gap-4 py-2 border-b hover: hover:bg-gray-100 transition duration-200 p-2 rounded">
            <div className="col-span-1">{item?.c_agency_id}</div>
            <div className="col-span-3 ml-[-60px]">{item?.c_agency_name?.length > 60 ? `${item?.c_agency_name.slice(0, 60)}...` : item?.c_agency_name}</div>
