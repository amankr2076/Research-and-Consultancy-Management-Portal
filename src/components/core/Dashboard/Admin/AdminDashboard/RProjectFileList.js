...
const RProjectFileList = () => {
  ...
  return (
    <div className='w-[1200px] ml-[-100px] rounded-lg shadow-lg py-8 px-8'>
        <h2 className='font-bold mb-4'>All Research Project Files</h2>
      ...
      {filesData.map((item,index) => (
        <div key={index}>
          ...
          {item?.r_file_id && <div className="grid grid-cols-6 gap-4 py-2 border-b hover: hover:bg-gray-100 transition duration-200 p-2 rounded">
            <div className="col-span-1">{item?.r_file_id}</div>
            <div className="col-span-3 ml-[-85px]">{item?.r_project_title?.length > 60 ? `${item?.r_project_title.slice(0, 60)}...` : item?.r_project_title}</div>
            <div className="col-span-2 flex gap-2">
                    <PaperClipIcon aria-hidden="true" className="h-5 w-5 mt-1 flex-shrink-0 text-gray-400" />
                    <a href={item?.r_proposal_file}
                    target="_blank" 
...
export default RProjectFileList