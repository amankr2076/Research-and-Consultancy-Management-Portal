...
const RProjectFundDetails = ({fund_details,setIsChoosen,type}) => {
  return (
    <div>
      ...
      {type==="subHeads" &&<div className="grid grid-cols-4 font-bold border-b-2 pb-2 mt-6">
        <div>Subhead Name</div>
      ...
      {fund_details.map((item,index) => (
        ...
        {type==="fundReleased" && <div className="grid grid-cols-3 gap-4 py-2 border-b">
          ...
          {type==="subHeads" && <div className="grid grid-cols-4 gap-4 py-2 border-b">
            <div>{item?.r_subhead_name}</div>
            <div>₹ {item?.allocated_funds}</div>
            <div>₹ {(item?.allocated_funds)-(item?.fund_balance)}</div>
...
export default RProjectFundDetails